...
export async function completeBattleJourneyStage(
  ...
  if (stageNumber === 3) {
    ...
    await writeJourneySave({ cpuId: 'todd', stageNumber: 1, updatedAt: Date.now() });
    return { nextStageNumber: 1, didCompleteCpuArc: true };
  }
  ...
  await writeJourneySave({ cpuId: 'todd', stageNumber: nextStageNumber, updatedAt: Date.now() });
  return { nextStageNumber, didCompleteCpuArc: false };
}
