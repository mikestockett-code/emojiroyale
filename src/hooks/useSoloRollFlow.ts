...
import type { SoloRollFlowParams, SoloRollFlowViewModel, SoloRollPhase } from './soloRollTypes';

export function useSoloRollFlow({
  board,
  onCommitRoll,
  onEnterRollMode,
  onExitRollMode,
  disabled = false,
}: SoloRollFlowParams): SoloRollFlowViewModel {
  const [phase, setPhase] = React.useState<SoloRollPhase>('inactive');
...
export type { SoloRollFlowViewModel as RollFlowViewModel } from './soloRollTypes';
